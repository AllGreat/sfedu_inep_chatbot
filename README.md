**Предыстория**
2022-2023 года
Во время проектной деятельности между подразделениями во время учебы Я был назначен менеджером проекта. Из имеющихся заказов я выбрал для себя самый интересный, а именно создание Telegram-бота, и набрал команду из энтузиастов со своего факультета.
В процессе были распределены роли и обязанности по пожеланиям и имеющимся навыкам. По ходу разработки стало яснее кто действительно трудится, а кто записался в проект, чтобы получить зачёт, но их мы трогать не стали. 
Предупреждая вопрос, на тот момент я только знакомился с глубоким программированием и толком не был знаком с GitHub, поэтому писал код на своём компьютере в среде, а для комитов отсылал его  коллеге, который позже загружал его в Git.
